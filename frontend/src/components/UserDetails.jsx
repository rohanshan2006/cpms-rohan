  ...
  const fetchCurrentUserData = async () => {
    try {
      ...
      if (completeProfileReq) {
        if (!(userId === response.data.id)) navigate('/404', { replace: true })
        ...
        if (response.data.isProfileCompleted === "true" || response.data.isProfileCompleted === true) {
          if (response.data.role === "student") navigate('/student/dashboard', { replace: true })
          if (response.data.role === "tpo_admin") navigate('/tpo/dashboard', { replace: true })
          if (response.data.role === "management_admin") navigate('/management/dashboard', { replace: true })
        }
      }
        ...
        if (response.data.msg) {
          ...
          if (completeProfileReq) {
            if (response.data.msg === "Data Updated Successfully!") {
              const role = currentUserData.role || response.data.role;
              if (role === "student") navigate('/student/dashboard', { replace: true });
              else if (role === "tpo_admin") navigate('/tpo/dashboard', { replace: true });
              else if (role === "management_admin") navigate('/management/dashboard', { replace: true });
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="flex justify-center items-center gap-3 mt-4">
                  ...
                  <Button variant="outline-primary" size="lg" onClick={() => {
                    ...
                    else if (currentUserData.role === 'tpo_admin') navigate('/tpo/dashboard');
                    else if (currentUserData.role === 'management_admin') navigate('/management/dashboard');
                    else navigate(-1);
                  }}>
                    Home
                  </Button>
