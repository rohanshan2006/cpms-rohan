...
const MOCK_NOTICES = [
  ...
  {
    _id: "mock-2",
    title: "Capgemini registration closes tomorrow",
    createdAt: new Date().toISOString(),
  },
  {
    _id: "mock-3",
    title: "Infosys OA results published",
    createdAt: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000).toISOString(),
  },
  {
    _id: "mock-4",
    title: "Coforge interview shortlist released",
...
function NoticeBox() {
  ...
  const finalNotices = noticesData && noticesData.length > 0 ? noticesData : MOCK_NOTICES;
            ...
            {finalNotices.map((notice, index) => (
              <div key={index} className="py-2 h-fit">
                <Link
                  ...
                  target={notice?._id.startsWith('mock') ? '_self' : '_blank'}
                >
                  {notice?.title}
                  {/* Show the badge if the notice is within 2 days */}
                  {((new Date() - new Date(notice?.createdAt)) / (1000 * 60 * 60 * 24) <= 2 || notice?._id.startsWith('mock')) && (
