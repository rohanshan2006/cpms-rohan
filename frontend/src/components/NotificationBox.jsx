...
const MOCK_NOTIFICATIONS = [
  {
    ...
    year: 4,
    jobs: [
      { jobId: "accenture-job", jobTitle: "Associate Software Engineer", companyName: "Accenture", status: "interview" }
    ]
  },
  {
    id: "mock-stud-2",
    ...
    year: 4,
    jobs: [
      { jobId: "capgemini-job", jobTitle: "Analyst", companyName: "Capgemini", status: "hired" }
    ]
  },
  {
    id: "mock-stud-3",
    ...
    year: 3,
    jobs: [
      { jobId: "infosys-job", jobTitle: "Systems Engineer", companyName: "Infosys", status: "interview" }
    ]
  }
];
...
function NotificationBox() {
  ...
  const finalNotifications = notify && notify.length > 0 ? notify : MOCK_NOTIFICATIONS;
        ...
        {loading ? (
          ...
          <div className="relative h-72 overflow-hidden">
            <div className="absolute bottom-0 w-full flex flex-col gap-2 h-full animate-scrollUp">
              {finalNotifications.map((student, studentIndex) => (
                <Link
                  key={studentIndex}
                  ...
                  target={student.id.startsWith('mock') ? '_self' : '_blank'}
                >
                  <div className="p-2 h-fit rounded bg-yellow-200 border-2 border-black transition-all duration-200 ease-in-out hover:bg-yellow-300">
                    <span className='text-sm text-black'>
                      <strong>{student.studentName}</strong> from
                      <b>
                    ...
                    <div className="flex flex-col gap-0.5 mt-1">
                      {student.jobs.map((job, jobIndex) => (
                        <Link
                          ...
                          target={student.id.startsWith('mock') ? '_self' : '_blank'}
                        >
                          <div className="py-1 h-fit text-xs text-slate-800">
                            {job?.jobTitle} at <strong>{job?.companyName}</strong>
                            <span className='mx-2 px-1.5 py-0.5 rounded bg-blue-100 text-blue-800 text-[10px] uppercase font-bold'>
                              Status: {job?.status}
                            </span>
                          </div>
                        </Link>
